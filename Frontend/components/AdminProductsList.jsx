...
function AdminProductsList({id, title, imageUrl, category, price, description, quantity}) {
  ...
  return (
    < >
        <div className="relative w-[95%] h-[200px] my-2">
          <Image src={imageUrl} alt={title} layout="fill" objectFit="cover" />
        </div>
        <div className="col-span-2">
      <article className="" dangerouslySetInnerHTML={{ __html: description }} 
      />
        </div>
        ...
        <div className="">
        <Link href={{ pathname: `/admin/add-product`, query: { prodId: id, title: title, imageUrl: imageUrl, category: category, price: price, description: description, quantity: quantity }}}> 
